...
class AuctionRepositoryTest {
    ...
    void setUp() {
        ...
        art = new Art("Starry Paint", "Painting style replica", 200.0, ItemCondition.GOOD, "img1", seller, "Vincent", 2024, "Oil", "50x50", false);
        itemRepository.save(art);

        electronics = new Electronics("LG Screen", "4K monitor screen", 300.0, ItemCondition.NEW, "img2", seller, "LG", "27GP850", 12);
        itemRepository.save(electronics);

        LocalDateTime now = LocalDateTime.now();
        auctionArt = new Auction("Rare Art Masterpiece", seller, art, 200.0, now.minusDays(1), now.plusDays(2));
        auctionArt.open();
        auctionRepository.save(auctionArt);

        auctionElec = new Auction("LG Monitor Sale", seller, electronics, 300.0, now.minusDays(2), now.minusMinutes(5));
    ...
    void testFindByStatusAndEndTimeBefore() {
        ...
        List<Auction> expired = auctionRepository.findByStatusAndEndTimeBefore(AuctionStatus.PENDING, checkTime);
    ...
    void testSearchPaged() {
        Page<Auction> pagedResult = auctionRepository.searchPaged("", null, Art.class, PageRequest.of(0, 10));
