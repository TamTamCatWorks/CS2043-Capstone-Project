...
public class LoadDatabase {
    ...
    return args -> {
      User bidder =
          new User(
              "testbidder",
              "test1@example.com",
              passwordEncoder.encode("hashed123"),
              "Test Bidder",
              3000.0);
  ...
  @Bean
  @Order(1)
  CommandLineRunner initItems(UserRepository userRepository, ItemRepository itemRepository) {
    return args -> {
      User seller = userRepository.findById(Objects.requireNonNull(SELLER_ID)).orElseThrow();
  ...
  @Bean
  @Order(2)
  CommandLineRunner initAuction(
      UserRepository userRepository,
      ItemRepository itemRepository,
      AuctionRepository auctionRepository) {
    return args -> {
      User seller = userRepository.findById(Objects.requireNonNull(SELLER_ID)).orElseThrow();
      Item item = itemRepository.findById(Objects.requireNonNull(ITEM_ID)).orElseThrow();
  ...
  @Bean
  @Order(3)
  CommandLineRunner initBidTransaction(
      UserRepository userRepository,
      AuctionRepository auctionRepository,
    ...
    return args -> {
      ...
      transactionTemplate.execute(
          status -> {
            User bidder = userRepository.findById(Objects.requireNonNull(BIDDER_ID)).orElseThrow();
            Auction auction =
                auctionRepository.findById(Objects.requireNonNull(AUCTION_ID)).orElseThrow();
            auction.open();

            bidder.holdFunds(2000);
            BidTransaction bidTransaction =
                new BidTransaction(auction, bidder, 2000, BidTransaction.BidType.MANUAL);
