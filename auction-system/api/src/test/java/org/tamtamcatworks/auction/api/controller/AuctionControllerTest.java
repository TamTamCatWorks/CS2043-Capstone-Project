...
class AuctionControllerTest {
  ...
  @MockBean private AutoBidService autoBidService;

  @Test
  void testGetAuctionEndpoint() throws Exception {
    AuctionResponse resp =
        new AuctionResponse(
            "auc123",
            "Comic Sale",
            "seller123",
  ...
  void testOpenAuctionEndpoint() throws Exception {
    ...
    mockMvc
        .perform(patch("/auctions/auc123/open"))
        .andExpect(status().isOk())
  ...
  void testPlaceBidEndpoint() throws Exception {
    ...
    BidResponse resp =
        new BidResponse(
            "bid123", "auc123", "bidder123", "John Doe", 120.0, "MANUAL", LocalDateTime.now());

    when(bidService.placeBid(eq("auc123"), eq("bidder123"), any(BidRequest.class)))
        .thenReturn(resp);
    mockMvc
        .perform(
            post("/auctions/auc123/bids")
                .session(session)
  ...
  void testRegisterAutoBidEndpoint() throws Exception {
    ...
    AutoBidResponse resp =
        new AutoBidResponse("auto123", "auc123", "bidder123", 1500.0, 1000.0, true);

    when(autoBidService.register(eq("auc123"), eq("bidder123"), any(AutoBidRequest.class)))
        .thenReturn(resp);

    mockMvc
        .perform(
            post("/auctions/auc123/auto-bid")
                .session(session)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(req)))
        .andExpect(status().isCreated())
