...
public class NotificationBroadcastListener {
  ...
  /**
   * Broadcast new bid price to all subscribers watching this auction. Topic:
  ...
  /** Broadcast auction status change. Topic: /topic/auctions/{auctionId}/status */
  @EventListener
  public void onAuctionStatusChanged(AuctionEvent event) {
    ...
    if (event.newStatus() == org.tamtamcatworks.auction.model.AuctionStatus.CLOSED
        && event.leadingBidderId() != null) {
      messagingTemplate.convertAndSendToUser(
    ...
    if (event.newStatus() == org.tamtamcatworks.auction.model.AuctionStatus.CANCELLED
        && event.leadingBidderId() != null) {
      messagingTemplate.convertAndSendToUser(
          event.leadingBidderId(),
          "/queue/notifications",
