 ...
 *
 * <p>Also implements {@link WebSocketHandlerDecoratorFactory} so it can be registered directly in
 * {@link WebSocketConfig#configureWebSocketTransport}. The decorator intercepts {@code
 * afterConnectionEstablished} / {@code afterConnectionClosed} to automatically keep the registry in
 * sync with live sessions.
 *
 * <p>Only authenticated users (those whose {@link WebSocketSession#getPrincipal()} is non-null) are
 * tracked; anonymous connections are ignored.
 */
@Component
public class WebSocketSessionRegistry implements WebSocketHandlerDecoratorFactory {
  ...
  /**
   * Registers a session under the user's ID, resolved from the session principal (email). No-op if
    ...
    userRepository
        ...
        .ifPresent(
            user ->
                sessions
                    .computeIfAbsent(
                        user.getId(), id -> Collections.newSetFromMap(new ConcurrentHashMap<>()))
                    .add(session));
  }

  /**
   * Removes a session from the registry, cleaning up the user entry if it becomes empty. No-op if
   * the session is unauthenticated.
   */
