...
/**
 * Application-event listener that force-closes all active WebSocket sessions belonging to a user
 * whose account has just been suspended.
 *
 * <p>Listens for {@link UserSuspendedEvent} (published by {@link
 * org.tamtamcatworks.auction.service.member.UserService#suspendUser(String)} after the database
 * transaction commits).
 *
 * <p>For each live session found in {@link WebSocketSessionRegistry}:
 *
 * <ol>
 *   <li>A close frame with status {@code 1008 Policy Violation} and reason {@code "Account
 *       suspended"} is sent to the client.
 *   <li>The underlying WebSocket connection is closed.
 * </ol>
 *
...
public class SuspendedUserWsHandler {
   ...
   *
   * <p>Iterates over a snapshot of the user's sessions (to avoid {@link
   * java.util.ConcurrentModificationException} as the registry updates on close) and closes each
   * open session with an explanatory close frame.
   *
  ...
  public void onUserSuspended(UserSuspendedEvent event) {
    ...
    for (WebSocketSession session : snapshot) {
      ...
      try {
        ...
        log.warn(
            "Failed to close WebSocket session [{}] for user [{}]: {}",
            session.getId(),
            userId,
            e.getMessage());
      }
