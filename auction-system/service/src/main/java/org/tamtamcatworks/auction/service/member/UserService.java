...
public class UserService {
  ...
  public UserResponse registerByRequest(RegisterRequest registerRequest) {
    ...
    User user =
        userMapper.toEntity(registerRequest, passwordEncoder.encode(registerRequest.password()));
  ...
  public AdminDashboardResponse getDashboardStats() {
    ...
    long totalAdmins =
        userRepository.findAll().stream().filter(user -> user.getAdminProfile() != null).count();

    long totalAuctions = auctionRepository.count();

    return new AdminDashboardResponse(totalUsers, totalAdmins, totalAuctions);
  }
  ...
  public List<String> getAllAdminLogs() {
    ...
    return userRepository.findAll().stream()
        ...
        .flatMap(user -> user.getAdminProfile().getActionLog().stream())
        .toList();
  }
  ...
  public List<AdminAuditLogResponse> getAuditLogs() {
    ...
    return userRepository.findAll().stream()
        ...
        .flatMap(
            user ->
                user.getAdminProfile().getActionLog().stream()
                    .map(
                        log ->
