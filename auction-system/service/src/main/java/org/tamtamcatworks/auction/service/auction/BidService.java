...
public class BidService {
  ...
  public BidResponse placeBid(String auctionId, String bidderId, BidRequest request) {
    ...
    boolean rebiddingAgainstSelf =
        previousLeader != null && previousLeader.getId().equals(bidder.getId());
    double requiredAvailableBalance =
        rebiddingAgainstSelf ? request.amount() - currentPrice : request.amount();
    ...
    LocalDateTime snipeWindowStart = auction.getEndTime().minusSeconds(antiSnipe.windowSeconds());
    ...
    if (previousLeader != null && !previousLeader.getId().equals(bidder.getId())) {
      eventPublisher.publishEvent(
          new UserStateEvent(previousLeader.getId(), userMapper.toResponse(previousLeader)));
    }
  ...
  public List<BidResponse> findByAuction(String auctionId) {
    Auction auction =
        auctionRepository
            .findById(auctionId)
            .orElseThrow(() -> new NoSuchElementException("Auction not found."));
    return bidRepository.findByAuctionOrderByCreationDateAsc(auction).stream()
  ...
  public List<BidResponse> findByBidder(String bidderId) {
    User bidder =
        userRepository
            .findById(bidderId)
            .orElseThrow(() -> new NoSuchElementException("Bidder not found."));
    return bidRepository.findByBidderOrderByCreationDateAsc(bidder).stream()
        .map(bidMapper::toResponse)
        .collect(Collectors.toList());
  }
}
