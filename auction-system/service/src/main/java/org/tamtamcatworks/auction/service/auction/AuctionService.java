...
public class AuctionService {
  ...
  public Auction create(
    ...
    return auctionRepository.save(
        new Auction(title, seller, item, startingPrice, startTime, endTime));
  }

  /**
   * Creates an auction for an existing item using the request DTO form. Use this overload from
   * API/controller paths; keep the parameter-based overload for internal callers.
   */
  @Transactional
  ...
  public Auction close(String auctionId) {
    ...
    if (leadingBidder != null && currentPrice > 0) {
      ...
      eventPublisher.publishEvent(
          new UserStateEvent(leadingBidder.getId(), userMapper.toResponse(leadingBidder)));
      ...
      eventPublisher.publishEvent(
          new UserStateEvent(seller.getId(), userMapper.toResponse(seller)));
    }
    Auction saved = auctionRepository.save(auction);
    eventPublisher.publishEvent(
        new AuctionEvent(
  ...
  @Transactional(readOnly = true)
  public List<AuctionResponse> searchResponses(
      String keyword, AuctionStatus status, String category) {
  ...
  @Transactional(readOnly = true)
  public org.tamtamcatworks.auction.shared.response.PageResponse<AuctionResponse>
      searchResponsesPage(
          String keyword, AuctionStatus status, String category, Pageable pageable) {
    String normalizedKeyword = keyword != null ? keyword.trim() : "";
    String normalizedCategory = category != null ? category.trim() : "";
    ...
    if (normalizedCategory != null
        ...
        && !"all categories".equalsIgnoreCase(normalizedCategory)) {
      try {
    ...
    Page<Auction> page =
        auctionRepository.searchPaged(normalizedKeyword, status, itemTypeClass, pageable);
