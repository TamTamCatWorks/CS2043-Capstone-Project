 ...
 *
 * <ol>
 *   <li>After a bid is accepted and committed ({@code BidEvent}), this listener fires.
 *   <li>Load <b>all</b> active auto-bids for the auction (including the current leader's) into a
 *       {@link PriorityQueue} ordered by {@code maxBid DESC}, tiebreaker: {@code creationDate ASC}
 *       (earlier registration wins ties).
 *   <li>Poll {@code best} = rank 1.
 *   <li><b>Case 3</b>: if {@code best.maxBid < currentPrice + increment} → deactivate only {@code
 *       best} and stop. The next-best is intentionally left active so it gets a fresh chance when
 *       the next {@code BidEvent} fires.
 *   <li><b>Cascade-termination guard</b>: if {@code best} is already the current leader, scan the
 *       remaining queue and deactivate any entry that can no longer meet {@code minimumNext}, then
 *       return without placing a new bid. This is what stops the cascade after at most two {@code
 *       onBidPlaced} invocations.
 *   <li><b>Tie drain</b>: deactivate all peers that share the same {@code maxBid} as {@code best}
 *       in a single pass — no cascade across tied bidders.
 *   <li>Compute {@code winningPrice}:
 *       <ul>
 *         <li>No remaining competitor → {@code currentPrice + increment} (Case 1).
 *         <li>Competitor present → {@code min(best.maxBid, second.maxBid + increment)} (Case 2).
 *       </ul>
 *   <li>Call {@link #executeAutoBid} for {@code best} at {@code winningPrice}.
 * </ol>
 *
...
public class AutoBidService {
  ...
  private static final Comparator<AutoBid> AUTO_BID_ORDER =
      Comparator.comparingDouble(AutoBid::getMaxBid)
          .reversed()
   ...
   *
   * <ul>
   *   <li><b>Non-leader (new or updated)</b>: if {@code maxBid >= minimumNext}, place an immediate
   *       bid at {@code minimumNext} via {@link #executeAutoBid}. The resulting {@code BidEvent}
   *       will trigger {@link #onBidPlaced}, which resolves any competing auto-bids in a single
   *       pass.
   *   <li><b>Leader updating maxBid</b>: no immediate bid needed — they already hold the top
   *       position. A synthetic {@code BidEvent} is published so {@link #onBidPlaced} re-evaluates
   *       competitors that may have been capped against the old ceiling.
   *   <li><b>Fresh registration by the current leader</b>: no action required.
   * </ul>
   *
  ...
  public AutoBidResponse register(String auctionId, String bidderId, AutoBidRequest request) {
    ...
    // ── Post-save dispatch ────────────────────────────────────────────────
    String leaderId =
        auction.getLeadingBidder() != null ? auction.getLeadingBidder().getId() : null;
  ...
  /**
   * Listens for {@link BidEvent} after transaction commit and resolves all active auto-bids in a
   * single pass (proxy/Vickrey-style).
   *
   * <p>{@code AFTER_COMMIT} ensures this reads the price committed by the triggering transaction,
   * not a stale snapshot. {@code REQUIRES_NEW} runs the resolution in its own transaction so any
   * deactivations or bids it causes are isolated.
   *
   * <h4>Resolution steps</h4>
   *
   * <ol>
   *   <li>Build one {@link PriorityQueue} from all active auto-bids (including the current
   *       leader's), ordered {@code maxBid DESC, creationDate ASC}.
   *   <li>Poll {@code best} (rank 1).
   *       <ul>
   *         <li><b>Case 3</b> – {@code best.maxBid < minimumNext}: deactivate only {@code best} and
   *             return. The next candidate is preserved for the next {@code BidEvent}.
   *         <li><b>Cascade-termination guard</b> – if {@code best} is already the current leader:
   *             deactivate any remaining entries that can no longer meet {@code minimumNext}, then
   *             return without placing a new bid.
   *       </ul>
   *   <li><b>Tie drain</b>: deactivate every remaining entry that shares {@code best.maxBid} —
   *       collapses an N-way tie in one pass, no cascade.
   *   <li>Compute {@code winningPrice}:
   *       <ul>
   *         <li><b>Case 1</b> (no remaining competitor): {@code minimumNext}.
   *         <li><b>Case 2</b> (competitor present): {@code min(best.maxBid, second.maxBid +
   *             increment)}.
   *       </ul>
   *   <li>Call {@link #executeAutoBid} for {@code best} at {@code winningPrice}.
   * </ol>
  ...
  public void onBidPlaced(BidEvent event) {
    ...
    while (!queue.isEmpty() && Double.compare(queue.peek().getMaxBid(), best.getMaxBid()) == 0) {
  ...
  /**
   * Validates the auto-bidder's eligibility at execution time (status or balance may have changed
   * since registration), then delegates to {@link BidService}. Deactivates the auto-bid on any
   ...
   */
  @Transactional(propagation = Propagation.REQUIRES_NEW)
  public void executeAutoBid(String auctionId, String autoBidId, String bidderId, double amount) {
  ...
  private void executeResolvedAutoBid(
      BidEvent event, Auction auction, AutoBid best, AutoBid second, double minimumNext) {
    double winningPrice =
        second == null
            ? minimumNext
