...
public interface ItemMapper {
  ...
  @Mapping(target = "condition", expression = "java(art.getCondition().name())")
  @Mapping(
      target = "sellerId",
      expression = "java(art.getSeller() != null ? art.getSeller().getId() : null)")
  ItemResponse toResponse(Art art);

  @Mapping(target = "itemType", constant = "VEHICLE")
  @Mapping(target = "condition", expression = "java(vehicle.getCondition().name())")
  @Mapping(
      target = "sellerId",
      expression = "java(vehicle.getSeller() != null ? vehicle.getSeller().getId() : null)")
  ItemResponse toResponse(Vehicle vehicle);

  @Mapping(target = "itemType", constant = "ELECTRONICS")
  @Mapping(target = "condition", expression = "java(electronics.getCondition().name())")
  @Mapping(
      target = "sellerId",
      expression = "java(electronics.getSeller() != null ? electronics.getSeller().getId() : null)")
