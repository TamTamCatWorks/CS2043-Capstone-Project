 ...
 * <h3>Architecture</h3>
 *
 * <p>This controller owns <em>no search field</em>. The global header search in {@code
 * LayoutController} is the single entry point. This controller:
 *
 * <ul>
 *   <li>Receives its initial parameters via {@link #initWithParams(SearchParams)} called by {@code
 *       LayoutController} after each navigation or live-type debounce.
 *   <li>Owns the in-page refinements: status filter, category filter, sort combo, grid/list toggle,
 *       and pagination.
 *   <li>Calls back to {@code LayoutController.syncHeaderSearch()} whenever an in-page filter change
 *       alters the effective query or category, keeping the header in sync.
 * </ul>
 */
...
public class SearchResultsController extends BaseController {
  ...
  public void initialize() {
    ...
    firstPageButton.setOnAction(
        e -> {
          currentPage = 0;
          applySortAndRender();
        });
    lastPageButton.setOnAction(
        e -> {
  ...
  /**
   * Called by {@link org.tamtamcatworks.auction.client.controller.shell.LayoutController} whenever
   * a new search is triggered (from the header, a shelf "See all →" link, or a live-type debounce
   * while this view is already active).
   *
   * <p>Populates filter combos from {@code params} without triggering recursive back-syncs, then
   * calls {@link #executeSearch()}.
   */
  ...
  /**
   * Fetch from the backend using the current combo values and {@code currentQuery}, then pipe
   * results through {@link #applySortAndRender()}.
   */
  ...
  private void applySortAndRender() {
    ...
    List<AuctionResponse> pageItems =
        from < sortedResults.size() ? sortedResults.subList(from, to) : List.of();
  ...
  private List<AuctionResponse> applySortOrder(List<AuctionResponse> source) {
    ...
    Comparator<AuctionResponse> cmp =
        switch (sort) {
          ...
          case "Most Bids" -> Comparator.comparingDouble((AuctionResponse a) -> a.currentPrice())
              .reversed();
          ...
          case "Price: High to Low" -> Comparator.comparingDouble(
                  (AuctionResponse a) -> a.currentPrice())
              .reversed();
          default -> Comparator.comparing(
              (AuctionResponse a) -> a.startTime() != null ? a.startTime() : LocalDateTime.MIN,
  ...
  private void renderResults(List<AuctionResponse> items) {
    ...
    if (gridMode) {
      ...
      for (AuctionResponse a : items) auctionsListContainer.getChildren().add(createListCard(a));
    }
  }
  ...
  private HBox createListCard(AuctionResponse a) {
    ...
    if (a.imageUrl() != null && !a.imageUrl().isEmpty()) {
      try {
        imgView.setImage(new Image(a.imageUrl(), true));
  ...
  /**
   * Called whenever the user changes a filter inside this page. Pushes the effective query +
   * category back up to the header field.
   */
