...
final class NotificationMenuManager {
  ...
  void handleMenuOpened() {
    new Thread(
            () -> {
              ...
              Platform.runLater(
                  () -> {
                    currentNotifications.replaceAll(
                        notification ->
  ...
  private void startNotificationWebSocket() {
    SessionManager.subscribeToNotifications(
            notification -> Platform.runLater(() -> onNotificationReceived(notification)))
        .thenAccept(sub -> notificationSubscription = sub)
        .exceptionally(
            ex -> {
              System.err.println(
                  "Notification WebSocket failed: " + (ex != null ? ex.getMessage() : "?"));
              return null;
            });
  }
