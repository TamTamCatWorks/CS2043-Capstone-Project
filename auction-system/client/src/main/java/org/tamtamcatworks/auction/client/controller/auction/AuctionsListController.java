 ...
 *
 * <ul>
 *   <li>Fetch all auctions once and distribute them across 8 named shelf rows.
 *   <li>Delegate "See all →" clicks to {@link
 *       org.tamtamcatworks.auction.client.controller.shell.LayoutController#navigateToSearchResults}
 *       so the global header stays in sync and the correct SearchParams are forwarded to the
 *       search-results page.
 * </ul>
 *
 * <h3>No search bar</h3>
 *
 * <p>This controller owns <em>no</em> search field. The global header search in {@code
 * dashboard-layout.fxml} is the single search entry point.
 */
...
public class AuctionsListController {
  ...
  private void renderShelves(List<AuctionResponse> source) {
    ...
    List<AuctionResponse> endingSoon =
        source.stream()
            .filter(this::isEndingSoon)
            .sorted(
                Comparator.comparing(a -> a.endTime() != null ? a.endTime() : LocalDateTime.MAX))
            .limit(12)
            .collect(Collectors.toList());

    List<AuctionResponse> newArrivals =
        source.stream()
            .filter(this::isNew)
            .sorted(
  ...
  @FXML
  private void onSeeAllFeatured() {
    navigateTo(SearchParams.forStatus("ACTIVE"));
  }

  @FXML
  private void onSeeAllEndingSoon() {
    navigateTo(new SearchParams("", "All categories", "ACTIVE", "Ending Soon", 0));
  }

  @FXML
  private void onSeeAllNew() {
    navigateTo(new SearchParams("", "All categories", "ALL", "Newest First", 0));
  }

  @FXML
  private void onSeeAllPopular() {
    navigateTo(new SearchParams("", "All categories", "ACTIVE", "Most Bids", 0));
  }

  @FXML
  private void onSeeAllArt() {
    navigateTo(SearchParams.forCategory("Art"));
  }

  @FXML
  private void onSeeAllElectronics() {
    navigateTo(SearchParams.forCategory("Electronics"));
  }

  @FXML
  private void onSeeAllVehicles() {
    navigateTo(SearchParams.forCategory("Vehicle"));
  }

  @FXML
  private void onSeeAllOther() {
    navigateTo(SearchParams.forCategory("Other"));
  }
