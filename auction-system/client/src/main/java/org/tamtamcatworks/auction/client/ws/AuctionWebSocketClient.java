...
public class AuctionWebSocketClient {
    ...
    stompClient.connectAsync(
        ...
        new StompSessionHandlerAdapter() {
          ...
          public void afterConnected(
              @NonNull StompSession stompSession, @NonNull StompHeaders connectedHeaders) {
          ...
          public void handleException(
              @NonNull StompSession stompSession,
          ...
          public void handleTransportError(
              @NonNull StompSession stompSession, @NonNull Throwable exception) {
            future.completeExceptionally(exception);
          }
        });
   ...
   * @return the subscription handle, or null if not connected
   */
  public StompSession.Subscription subscribeToStatus(
      String auctionId, Consumer<AuctionStatusUpdate> onStatusUpdate) {
    if (session == null || !session.isConnected()) {
      return null;
    }
   ...
   * @return the subscription handle, or null if not connected
   */
  public StompSession.Subscription subscribeToNotifications(
      Consumer<NotificationResponse> onNotification) {
    if (session == null || !session.isConnected()) {
      return null;
    }
   ...
   * @return the subscription handle, or null if not connected
   */
  public StompSession.Subscription subscribeToUserState(
      String userId, Consumer<UserResponse> onUserState) {
    if (session == null || !session.isConnected()) {
      return null;
    }

    return session.subscribe(
        "/topic/user-state/" + userId,
        new StompFrameHandler() {
          @Override
          public @NonNull Type getPayloadType(@NonNull StompHeaders headers) {
