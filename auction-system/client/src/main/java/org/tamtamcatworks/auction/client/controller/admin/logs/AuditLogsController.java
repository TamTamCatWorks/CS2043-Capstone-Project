...
@Route(fxml = "/fxml/admin/logs/audit-logs.fxml", layout = "/fxml/admin/layout/admin-layout.fxml")
public class AuditLogsController extends BaseController {
  ...
  private final PaginatedTableView<AdminAuditLogResponse> paginatedTable =
  ...
  private void buildToolbar() {
    ...
    toolbar.getRefreshButton().setOnAction(event -> loadLogs());
  }
  ...
  private void buildTable() {
    ...
    table
        .getColumns()
        .addAll(
            TableColumnFactory.createStringColumn("Admin", AdminAuditLogResponse::adminName),
            TableColumnFactory.createStringColumn("Action", AdminAuditLogResponse::action),
            TableColumnFactory.createStringColumn("Target", AdminAuditLogResponse::target),
            TableColumnFactory.createStringColumn("Timestamp", log -> log.timestamp().toString()));
    ...
    AsyncExecutor.execute(
        () -> {
          ...
          javafx.application.Platform.runLater(
              () ->
                  paginatedTable.getTableView().setItems(FXCollections.observableArrayList(logs)));
        },
        () -> {
          loadingOverlay.hide();

          Toast.show("Audit logs loaded", ToastType.INFO);
        },
        () -> {
          loadingOverlay.hide();

          ErrorDialog.show("Failed to load audit logs");
        });
  }
}
